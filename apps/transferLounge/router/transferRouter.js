...
class TransferRouter {
  ...
  setRouter () {
                ...
                if (Array.isArray(files[filesKeys[i]])) {
                  for (let j of files[filesKeys[i]]) {
                    ...
                    if (/\.pdf$/.test(j.originalFilename)) {
                      try {
                        await imageReader.pdfToJpg(clientFolder + '/' + list[i] + '/' + j.originalFilename, true);
                      } catch {}
                    }
                  }
                } else {
                  ...
                  if (/\.pdf$/.test(files[filesKeys[i]].originalFilename)) {
                    try {
                      await imageReader.pdfToJpg(clientFolder + '/' + list[i] + '/' + files[filesKeys[i]].originalFilename, true);
                    } catch {}
                  }
                }
              }
              ...
              for (let i = 0; i < list.length; i++) {
                if (Array.isArray(files[filesKeys[i]])) {
                  for (let j of files[filesKeys[i]]) {
                    ...
                    if (/\.pdf$/.test(j.originalFilename)) {
                      try {
                        await imageReader.pdfToJpg(aspirantFolder + '/' + list[i] + '/' + j.originalFilename, true);
                      } catch {}
                    }
                  }
                } else {
                  ...
                  if (/\.pdf$/.test(files[filesKeys[i]].originalFilename)) {
                    try {
                      await imageReader.pdfToJpg(aspirantFolder + '/' + list[i] + '/' + files[filesKeys[i]].originalFilename, true);
                    } catch {}
                  }
                }
              }
    ...
    router.post([ "/userBinary" ], async function (req, res) {
      res.set({
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, GET, OPTIONS, HEAD",
    ...
    router.post([ "/userPhoto" ], async function (req, res) {
      res.set({
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, GET, OPTIONS, HEAD",
    ...
    router.post([ "/userKey" ], async function (req, res) {
      res.set({
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, GET, OPTIONS, HEAD",
    ...
    router.post([ "/messageLog" ], async function (req, res) {
      ...
      try {
        ...
        if (!/alive/gi.test(channel)) {
          setQueue(() => {
            slack_bot.chat.postMessage({ text: slackText, channel: (channel === "silent" ? "#error_log" : channel) }).catch((err) => { console.log(err); });
          }, 0);
        }
    ...
    router.post([ "/getClients", "/getDesigners", "/getProjects", "/getContents", "/getBuilders", "/getAspirants" ], async function (req, res) {
      res.set({
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, GET, OPTIONS, HEAD",
    ...
    router.post([ "/rawImageParsing" ], async function (req, res) {
      ...
      try {
        ...
        await logger.error(e, req).catch((e) => { console.log(e); });
        res.send(JSON.stringify({ error: e.message }));
      }
    });
    
    router.post([ "/rawContentsSync" ], async function (req, res) {
      res.set({
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, GET, OPTIONS, HEAD",
      ...
      try {
        const rawcontentsSyncFunc = async function (MONGOC, logger) {
          try {
            ...
            const allProjects = await back.getProjectsByQuery({ desid: { $regex: "^d" } }, { selfMongo });
            ...
            const allClients = await back.getClientsByQuery({ $or: targetProjects.map((p) => { return { cliid: p.cliid } }) }, { selfMongo });
            const allDesigners = await back.getDesignersByQuery({ $or: targetProjects.map((p) => { return { desid: p.desid } }) }, { selfMongo });
        ...
        rawcontentsSyncFunc(mongo, logger).then((boo) => {
          if (boo) {
            return logger.cron("raw contents sync done");
          } else {
            return logger.error(e, req).catch((e) => { console.log(e); });
          }
