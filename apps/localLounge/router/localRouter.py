...
class LocalRouter:
    ...
    def setRouting(self):
        ...
        @app.post("/generalFileUpload")
        async def rou_post_generalFileUpload():
            headers = self.headers
            body = await decodeMultipart(request)
            staticConst = self.staticConst
            try:
                if not "toArr" in body:
                    raise Exception("invalid post")
                
                toArr = bytesJsonParse(body["toArr"])
                keyArr = list(body.keys())
                filteredKeyArr = []
                for k in keyArr:
                    if k != "toArr":
                        filteredKeyArr.append(k)

                filteredKeyArr = listSort(filteredKeyArr, lambda x: int(patternReplace(x, r"[^0-9]", "")))

                if len(filteredKeyArr) != len(toArr):
                    raise Exception("invalid post 2")

                for i in range(len(filteredKeyArr)):
                    thisPath = staticConst
                    if patternTest(r"^\/", toArr[i]):
                        thisPath += toArr[i]
                    else: 
                        thisPath += "/" + toArr[i]
                    thisPathArr = thisPath.split("/")
                    thisPathArr.pop()
                    thisFolderPath = "/".join(thisPathArr)
                    await shellExec("mkdir", [ "-p", thisFolderPath ])
                    await fileSystem("writeBinary", [ thisPath, body[filteredKeyArr[i]] ])

                return ({ "message": "done" }, 200, headers)
            except Exception as e:
                print(e)
                await alertLog("Local lounge 서버 문제 생김 (rou_post_generalFileUpload): " + str(e))
                return { "error": str(e) }