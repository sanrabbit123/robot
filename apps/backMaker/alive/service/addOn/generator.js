const ServiceSampleData = {
  ...
  // 서비스의 유형을 나타냄 (여기서는 "service")
  "kind": "service",
  // 서비스에 대한 세부 설정 정보
  "setting": {
    // 서비스의 좌표 설정 (등급, 종류, 제공 방식)
    "coordinates": {
      // 서비스 등급을 나타내는 x축 좌표 설정
      "x": {
        "M": "mini",    // M등급 (미니)
        "B": "basic",   // B등급 (기본)
        "P": "premium"  // P등급 (프리미엄)
      },
      // 서비스 종류를 나타내는 y축 좌표 설정
      "y": {
        "homeFurnishing": "홈퍼니싱",      // 홈퍼니싱 서비스
        "homeStyling": "홈스타일링",       // 홈스타일링 서비스
        "totalStyling": "토탈 스타일링",   // 토탈 스타일링 서비스
        "extraStyling": "엑스트라 스타일링" // 엑스트라 스타일링 서비스
      },
      // 서비스 제공 방식을 나타내는 z축 좌표 설정
      "z": {
        "online": "온라인",   // 온라인 서비스
        "offline": "오프라인" // 오프라인 서비스
      }
    },
    // 서비스의 전체 기간 (일 단위)
    "period": 45,
    // 서비스 제공 과정의 단계별 일정 정보
    "schedule": [
      {
        // 첫 번째 디자인 제안 단계
        "title": "1차 디자인 제안",
      ...
      },
      {
        ...
        // 이 단계가 진행되는 기간 (일 단위)
        "period": 6,
        // 이 단계의 순서 (여기서는 두 번째 단계, 수정 디자인과 같은 순서)
        "order": 1
      },
      {
        // 시공 견적 안내 단계
      ...
      },
      {
        ...
        // 이 단계가 진행되는 기간 (일 단위)
        "period": 15,
        // 이 단계의 순서 (여기서는 세 번째 단계, 시공 견적 안내와 같은 순서)
        "order": 2
      },
      {
        // 제품 구매 단계
      ...
      },
      {
        ...
        // 이 단계가 진행되는 기간 (일 단위)
        "period": 7,
        // 이 단계의 순서 (여기서는 네 번째 단계, 제품 구매와 같은 순서)
        "order": 3
      },
      {
        // 제작 가구 발주 단계
        ...
        // 이 단계가 진행되는 기간 (일 단위)
        "period": 7,
        // 이 단계의 순서 (여기서는 네 번째 단계, 제품 구매 및 패브릭 제작과 같은 순서)
        "order": 3
      },
      {
        // 입주 청소 단계
      ...
      },
      {
        ...
        // 이 단계가 진행되는 기간 (일 단위)
        "period": 4,
        // 이 단계의 순서 (여기서는 여섯 번째 단계, 설치 및 세팅과 같은 순서)
        "order": 5
      },
      {
        // 촬영 및 인터뷰 단계
