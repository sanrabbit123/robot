...
NumbersJs.prototype.mainDataRender = async function (matrixMode = false) {
  const instance = this;
...
NumbersJs.prototype.clientWhiteData = async function (cliid, requestNumber) {
  const instance = this;
...
NumbersJs.prototype.clientWhiteContents = async function (tong, cliid, requestNumber) {
  const instance = this;
...
NumbersJs.prototype.adsWhiteContents = async function (tong, data, startDate, endDate) {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight } = this;
...
NumbersJs.prototype.clientWhiteHistory = async function (tong, dataSet) {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight } = this;
  const { createNode, colorChip, withOut, findByAttribute, removeByClass, isMac, dateToString, stringToDate, cleanChildren, setQueue, blankHref, ajaxJson, stringToLink, variableArray, downloadFile, uniqueValue, sleep, equalJson, hexaJson } = GeneralJs;
  try {
    ...
    let maxWidth;
    let matrixDelta;
    let thisSource, thisMedium, thisCampain;
    let middleWidth;
    ...
    for (let obj of targetDetail) {
      thisSource = "-";
      thisMedium = "-";
      thisCampain = "-";
      if (/utm_source\=([^\&]+)/gi.test(obj.path)) {
        thisSource = /utm_source\=([^\&]+)/gi.exec(obj.path)[1];
      }
      if (/utm_medium\=([^\&]+)/gi.test(obj.path)) {
        thisMedium = /utm_medium\=([^\&]+)/gi.exec(obj.path)[1];
      }
      if (/utm_campaign\=([^\&]+)/gi.test(obj.path)) {
        thisCampain = /utm_campaign\=([^\&]+)/gi.exec(obj.path)[1];
      }
      obj.source = thisSource;
      obj.medium = thisMedium;
      obj.campaign = thisCampain;
    }
    ...
    for (let block of targetDetail) {
        ...
        children: [
          ...
          },
          {
            style: {
              ...
              background: eventDictionary[block.event.split("_")[0]]?.mode === "green" ? colorChip.green : colorChip.gray0,
              height: String(blockHeight) + ea,
              width: withOut(timeWidth + eventWidth + (pageNameWidth * 3) + (historyBlockMargin * 5), ea),
              borderRadius: String(3) + "px",
              textAlign: "center",
              justifyContent: "center",
              alignItems: "center",
      ...
      if (Array.isArray(this.clientDetailMatrix[num + 1])) {
        ...
        this.clientDetailMatrix[num + 1][5] = block.source;
        this.clientDetailMatrix[num + 1][6] = block.medium;
        this.clientDetailMatrix[num + 1][7] = block.campaign;
        this.clientDetailMatrix[num + 1][8] = block.title.replace(/\| 홈리에종$/gi, '').trim();
      }
...
NumbersJs.prototype.clientWhiteCard = function (cliid, requestNumber) {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight } = this;
          ...
          children: [
            {
              style: {
                display: "inline-block",
                position: "relative",
                width: "calc(calc(" + withOut(0, ea) + " / 5) * 3)",
                height: withOut(0, ea),
                overflow: "scroll",
            ...
            },
            {
              style: {
                display: "inline-block",
                position: "relative",
                width: "calc(calc(" + withOut(0, ea) + " / 5) * 2)",
                height: withOut(0, ea),
                overflow: "scroll",
                border: "1px solid " + colorChip.gray3,
                borderLeft: "",
...
NumbersJs.prototype.adsWhiteCard = function () {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight } = this;
  const { titleButtonsClassName, whiteCardClassName, whiteBaseClassName } = this;
...
NumbersJs.prototype.frontWhiteCard = function () {
  const instance = this;
...
NumbersJs.prototype.snsWhiteCard = function () {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight, entireMode } = this;
  const { titleButtonsClassName, whiteCardClassName, whiteBaseClassName } = this;
...
NumbersJs.prototype.queryWhiteCard = function () {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight, entireMode } = this;
...
NumbersJs.prototype.statisticsWhiteCard = function () {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight, entireMode } = this;
  const { titleButtonsClassName, whiteCardClassName, whiteBaseClassName } = this;
  const { createNode, colorChip, withOut, findByAttribute, removeByClass, isMac, dateToString, stringToDate, cleanChildren, setQueue, blankHref, ajaxJson, autoComma, zeroAddition, chartJsPatch, serviceParsing, hasQuery, removeQuery, appendQuery } = GeneralJs;
...
NumbersJs.prototype.contractWhiteCard = function () {
  const instance = this;
  const { ea, totalContents, grayBarWidth, belowHeight, entireMode } = this;
  const { titleButtonsClassName, whiteCardClassName, whiteBaseClassName } = this;
  const { createNode, colorChip, withOut, findByAttribute, removeByClass, isMac, dateToString, stringToDate, cleanChildren, setQueue, blankHref, ajaxJson, autoComma, zeroAddition, chartJsPatch, serviceParsing, hasQuery, removeQuery, appendQuery } = GeneralJs;
...
NumbersJs.prototype.mprPannel = async function () {
  const instance = this;
...
NumbersJs.prototype.mprWhiteResize = async function () {
  const instance = this;
...
NumbersJs.prototype.mprSearchEvent = async function () {
  const instance = this;
...
NumbersJs.prototype.mprExtractEvent = async function () {
  const instance = this;
...
NumbersJs.prototype.launching = async function () {
  const instance = this;
