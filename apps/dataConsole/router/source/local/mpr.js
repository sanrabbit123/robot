...
MprJs.prototype.adsWhiteContents = async function (tong, data, startDate, endDate) {
  ...
  try {
    ...
    for (let obj of timeMatrix) {
      ...
      newDataArray = obj.data.filter((o) => { return o.kinds !== "naver" && o.kinds !== "meta" });
            ...
            for (let i = 0; i < timeDelta + 1; i++) {
              ...
              if (thisRows.length === 1 && thisBasics.length === 1) {
                ...
                timeMatrix.push({
                  ...
                  meta: thisRow.data.users.detail.source.cases.filter((c) => { return /instagram/gi.test(c.case) || /facebook/gi.test(c.case) || /meta/gi.test(c.case) }).reduce((acc, curr) => { return acc + curr.value }, 0),
