...
DesignerJs.prototype.cardViewMaker = function () {
  ...
  return async function (e) {
    ...
    if (instance.totalFather !== null) {
      ...
      //info style
      styles = [];
      for (let i = 0; i < DataPatch.designerCardViewStandard().info.length; i++) {
        temp = {
          position: "absolute",
          fontSize: String(fontSize) + ea,
          fontWeight: String(500),
          top: String(startTop + (lineHeight * (i + 1)) + (DataPatch.designerCardViewStandard().exceptionHeight[i] ? exceptionMargin : 0)) + ea,
          left: String(intend) + ea,
          width: String(totalWidth) + ea,
          color: "#404040",
      ...
      for (let obj of cases) {
        if (num !== 0) {
          ...
          //sub info
          for (let j = 0; j < DataPatch.designerCardViewStandard().info.length; j++) {
            div_clone2 = GeneralJs.nodes.div.cloneNode(true);
            div_clone2.classList.add("father_" + DataPatch.designerCardViewStandard().info[j]);
            div_clone2.textContent = obj[DataPatch.designerCardViewStandard().info[j]];
