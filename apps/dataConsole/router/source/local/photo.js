...
PhotoJs.prototype.standardBar = function (standard) {
  const instance = this;
...
PhotoJs.prototype.spreadData = async function (search = null) {
  ...
  try {
    ...
    if (search === null || search === '' || search === '-') {
      projects = JSON.parse(await GeneralJs.ajaxPromise("limit=200&where=" + JSON.stringify({ "$and": [ { "desid": { "$regex": "^d" } }, { "process.status": { "$not": { "$regex": "^드" } } } ] }), "/getPhotos"));
    } else {
...
PhotoJs.prototype.cardViewMaker = function () {
  const instance = this;
...
PhotoJs.prototype.whiteCancelMaker = function (callback = null, recycle = false) {
  const instance = this;
  return function (e) {
...
PhotoJs.prototype.whiteViewMaker = function (index) {
  const instance = this;
  return function (e) {
    let tempFunc;
...
PhotoJs.prototype.rowViewMaker = function () {
  const instance = this;
  return function (e) {
    if (instance.totalFather !== null) {
      instance.totalFather.style.zIndex = String(-1);
...
PhotoJs.prototype.returnValueEventMaker = function () {
  const instance = this;
  return async function (e) {
    let pastObj;
    let textTargets;
...
PhotoJs.prototype.reportScrollBox = function (data, motherWidth) {
  const instance = this;
...
PhotoJs.prototype.reportContents = function (data, mother, loadingIcon) {
  const instance = this;
...
PhotoJs.prototype.reportViewMakerDetail = function (recycle = false) {
  const instance = this;
  const zeroAddition = function (number) {
    if (number < 10) {
      return "0" + String(number);
...
PhotoJs.prototype.reportViewMaker = function () {
  const instance = this;
  return function (e) {
    let tempFunc;
    if (GeneralJs.stacks.whiteBox !== 1) {
...
PhotoJs.prototype.addTransFormEvent = function () {
  const instance = this;
...
PhotoJs.prototype.addSearchEvent = function () {
  const instance = this;
  const input = this.searchInput;
  input.addEventListener("keypress", this.makeSearchEvent(null));
}

PhotoJs.prototype.backGrayBar = function () {
  const instance = this;
  let div_clone;
  let style;
  let ea = "px";
...
PhotoJs.prototype.extractViewMakerDetail = function (recycle = false, link) {
  const instance = this;
  try {
    return function () {
      let div_clone;
...
PhotoJs.prototype.extractViewMaker = function (link) {
  const instance = this;
  return function (e) {
    let tempFunc;
    if (GeneralJs.stacks.whiteBox !== 1) {
...
PhotoJs.prototype.addExtractEvent = function () {
  const instance = this;
...
PhotoJs.prototype.makeClipBoardEvent = function (id) {
  const instance = this;
  return async function (e) {
    if (e.cancelable) {
      e.preventDefault();
...
PhotoJs.prototype.makeImportantEvent = function (id, update = true) {
  const instance = this;
  const cookies = GeneralJs.getCookiesAll();
  return async function (e) {
    if (e.cancelable) {
...
PhotoJs.prototype.whiteResize = function () {
  const instance = this;
  this.resizeStack = 0;
  this.resizeFrom = 0;
  this.resizePopup = 0;
