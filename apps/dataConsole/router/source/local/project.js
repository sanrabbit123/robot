...
ProjectJs.prototype.processViewMakerDetail = function (recycle = false) {
  const instance = this;
  const { ea } = this;
  const { ajaxJson, colorChip, createNode, withOut } = GeneralJs;
  try {
...
ProjectJs.prototype.processViewMaker = function () {
  const instance = this;
  return function (e) {
    ...
    if (GeneralJs.stacks.whiteBox !== 1) {
      if (instance.whiteBox !== null) {
        tempFunc = instance.whiteCancelMaker(instance.processViewMakerDetail(true), true);
        tempFunc();
      } else {
        tempFunc = instance.processViewMakerDetail(false);
        tempFunc();
      }
    }
  }
...
ProjectJs.prototype.addTransFormEvent = function () {
  ...
  up.addEventListener("click", this.processViewMaker());
