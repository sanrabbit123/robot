...
ProjectJs.prototype.filesViewMakerDetail = function (recycle = false) {
  const instance = this;
  const { ea } = this;
  const { ajaxJson, colorChip, createNode, withOut } = GeneralJs;
  try {
    return function () {
      ...
      createNode({
        ...
        child: {
          mode: "iframe",
          attribute: {
            src: window.location.protocol + "//" + window.location.host + "/file?entire=true&dataonly=true&mode=project",
          },
          style: {
            position: "absolute",
            display: "block",
...
ProjectJs.prototype.filesViewMaker = function () {
  const instance = this;
  return function (e) {
