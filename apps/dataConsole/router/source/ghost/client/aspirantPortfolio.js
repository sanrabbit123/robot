/<%patch%>/ {
  ...
  "meta": {
    "title": [
      "thisPerson",
      "return ('추가 포트폴리오 전송 | 홈리에종');"
    ],
    "description": [
      "thisPerson",
      "return ('추가 포트폴리오 전송 페이지 입니다! | 홈리에종');"
    ],
    "image": [
      "thisPerson",
...
const AspirantPortfolioJs = function () {
  this.mother = new GeneralJs();
}

AspirantPortfolioJs.binaryPath = FRONTHOST + "/middle/aspirant";

AspirantPortfolioJs.prototype.insertInitBox = function () {
  const instance = this;
  ...
  subTitleContents = "추가적으로 포트폴리오를 요청드립니다!";  
...
AspirantPortfolioJs.prototype.insertAspirantBox = function () {
  const instance = this;
    ...
    try {
      ...
      GeneralJs.stacks["addressEvent"] = async function (e) {
        try {
          ...
          await GeneralJs.ajaxJson({ message: "AspirantPortfolioJs.addressEvent : " + e.message }, BACKHOST + "/errorLog");
        }
      }
...
AspirantPortfolioJs.prototype.finalSubmit = function () {
  const instance = this;
  const inputClassName = "inputClassName";
  const agreeTargetClassName = "agreeTargetClassName";
...
AspirantPortfolioJs.prototype.launching = async function (loading) {
  const instance = this;
  try {
    ...
    await this.mother.ghostClientLaunching({
      ...
      base: {
        instance: this,
        binaryPath: AspirantPortfolioJs.binaryPath,
        subTitle: "",
      ...
      local: async () => {
        try {
          ...
          await GeneralJs.ajaxJson({ message: "AspirantPortfolioJs.launching.ghostClientLaunching : " + e.message }, BACKHOST + "/errorLog");
        }
      }
    });
    ...
    console.log(err);
    await GeneralJs.ajaxJson({ message: "AspirantPortfolioJs.launching 에러 일어남 => " + err.message }, BACKHOST + "/errorLog");
  }
}
