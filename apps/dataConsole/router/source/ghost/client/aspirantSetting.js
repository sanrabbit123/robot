/<%patch%>/ {
  ...
  "meta": {
    ...
    "description": [
      "thisPerson",
      "return ('세트 포트폴리오 전송 페이지 입니다! | 홈리에종');"
    ],
    "image": [
      "thisPerson",
...
const AspirantSettingJs = function () {
  this.mother = new GeneralJs();
}

AspirantSettingJs.binaryPath = FRONTHOST + "/middle/aspsetting";
    ...
    try {
      ...
      GeneralJs.stacks["addressEvent"] = async function (e) {
        try {
          ...
          await GeneralJs.ajaxJson({ message: "AspirantSettingJs.addressEvent : " + e.message }, BACKHOST + "/errorLog");
        }
      }
...
AspirantSettingJs.prototype.finalSubmit = function () {
  const instance = this;
  const inputClassName = "inputClassName";
  const agreeTargetClassName = "agreeTargetClassName";
...
AspirantSettingJs.prototype.launching = async function (loading) {
  const instance = this;
  try {
    ...
    await this.mother.ghostClientLaunching({
      ...
      base: {
        instance: this,
        binaryPath: AspirantSettingJs.binaryPath,
        subTitle: "",
      ...
      local: async () => {
        try {
          ...
          await GeneralJs.ajaxJson({ message: "AspirantSettingJs.launching.ghostClientLaunching : " + e.message }, BACKHOST + "/errorLog");
        }
      }
    });
    ...
    console.log(err);
    await GeneralJs.ajaxJson({ message: "AspirantSettingJs.launching 에러 일어남 => " + err.message }, BACKHOST + "/errorLog");
  }
}
