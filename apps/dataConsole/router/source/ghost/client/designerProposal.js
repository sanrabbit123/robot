...
DesignerProposalJs.prototype.insertFeeServiceBox = function () {
  const instance = this;
  const { withOut, returnGet, createNode, colorChip, isMac, svgMaker, serviceParsing } = GeneralJs;
  const { client, ea, media, osException, testMode } = this;
  const mobile = media[4];
  ...
  let infoMargin;
  let infoPhotoHeight;
  let infoTextBoxHeight;
  let infoTitleHeight;
  let infoTitlePadding;
  let infoTitleInnerMargin;
  let infoTitleSize;
  ...
  grayBoxHeight = <%% 320, 320, 311, 250, 43 %%>;
  ...
  descriptionSize = <%% 15, 15, 14, 13, 3.2 %%>;
  descriptionWeight = desktop ? 300 : 400;
  descriptionLineHeight = 1.6;
  descriptionTitleWeight = 700;

  mobileTitleLeft = <%% 7, 7, 7, 7, 7 %%>;
  mobileTitleTop = <%% 7, 7, 7, 7, 7 %%>;
  mobileTitleMarginBottom = <%% 7, 7, 7, 7, 7 %%>;
  mobileRightBoxPaddingTop = <%% 49, 49, 49, 49, 49 %%>;
  whiteBlockPaddingBottom = <%% 9, 9, 9, 9, 9 %%>;

  mobileGrayMarginTop = <%% 43, 43, 43, 43, 5 %%>;
  mobileGrayMarginLeft = <%% 6, 6, 6, 6, 5 %%>;
  mobilePictureBetween = <%% 12, 12, 12, 12, 12 %%>;
  mobileGrayMarginBottom = 10.5;

  firstWidth = <%% 200, 170, 182, 146, 25.5 %%>;
  secondWidth = <%% 300, 200, 182, 146, 25.5 %%>;
  infoMargin = <%% 10, 10, 10, 10, 1 %%>;
  infoPhotoHeight = <%% 170, 170, 170, 130, 26 %%>;
  infoTextBoxHeight = <%% 96, 96, 90, 80, 16.5 %%>;
  infoTitleHeight = <%% 54, 54, 50, 40, 5 %%>;
  infoTitlePadding = <%% 12, 12, 10, 10, 2 %%>;
    ...
    info: {
      ...
      description: [
        [
          ...
          <?? "3D, 콜라주, 구매 리스트" | "3D, 콜라주, 구매 리스트" | "3D, 콜라주, 구매 리스트" | "3D, 콜라주, 구매 리스트" | "3D, 콜라주, 구매표" ??>,
          "등등",
        ],
        [
        ...
        [
          <?? "주방 철거, 화장실 철거, 아일랜드형 싱크 시공" | "주방 철거, 화장실 철거, 아일랜드형 싱크 시공" | "주방 철거, 화장실 도기 시공" | "주방 철거, 화장실 도기 시공" | "주방, 화장실 철거" ??>,
  ...
  if (media[0]) {
    wordings.contents.description = [ "홈리에종은 선 기획 후 시공 방식으로 인테리어를 진행하기에, 디자이너의 인건비 개념인 디자인 비용을 먼저 받고, 그 디자인에 의해 나온 구매 리스트와 시공 범위에 따라 제품 구매와 시공을 진행합니다. 앞으로 소개될 금액은 모두 디자이너의 디자인 비용입니다." ];
  } else if (media[1]) {
  ...
  createNode({
    ...
    style: {
      ...
      right: desktop ? String(titleLeft) + ea : "",
    ...
    children: [
      {
        style: {
          ...
          backgroundPosition: <?? "50% 50%" | "74% 50%" | "50% 50%" | "64% 50%" | "74% 50%" ??>,
          backgroundImage: "url('" + DesignerProposalJs.binaryPath + "/" + wordings.info.image[0] + "')",
          borderRadius: String(5) + "px",
          boxShadow: "0px 6px 20px -9px " + colorChip.darkShadow,
          width: String(100) + '%',
          height: String(infoPhotoHeight) + ea,
        }
      },
      {
        text: wordings.info.description[0].join("\n"),
        style: {
          display: "flex",
          height: String(infoTextBoxHeight) + ea,
          width: String(100) + '%',
          textAlign: "center",
          justifyContent: "center",
          alignItems: "center",
          fontSize: String(infoDescriptionSize) + ea,
          fontWeight: String(500),
          lineHeight: String(1.5),
          color: colorChip.shadow,
          marginBottom: desktop ? "" : String(infoDescriptionMarginBottom) + ea,
        }
      },
      {
        style: {
        ...
        children: [
            ...
            children: [
              {
                ...
                style: {
                  display: "inline-block",
                  position: "relative",
                  fontSize: String(infoTitleSize) + ea,
                  fontWeight: String(500),
                  color: colorChip.green,
                  fontFamily: "graphik",
          ...
          },
          {
            style: {
              display: "inline-flex",
              position: "relative",
              width: withOut(infoTitleHeight - infoTitlePadding + infoTitleInnerMargin, ea),
              height: String(infoTitleHeight - infoTitlePadding) + ea,
              background: desktop ? colorChip.white : "",
              borderRadius: String(5) + "px",
              justifyContent: "center",
  ...
  createNode({
    ...
    children: [
      ...
      {
        text: wordings.info.description[1].join("\n"),
        style: {
          display: "flex",
          height: String(infoTextBoxHeight) + ea,
          width: String(100) + '%',
  ...
  createNode({
    ...
    children: [
      ...
      {
        text: wordings.info.description[2].join("\n"),
        style: {
          display: "flex",
          height: String(infoTextBoxHeight) + ea,
          width: String(100) + '%',
