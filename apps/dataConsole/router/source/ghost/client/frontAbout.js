...
FrontAboutJs.binaryPath = "/middle/curation";
...
FrontAboutJs.prototype.insertServiceBox = function () {
  const instance = this;
  const { ea, media } = this;
      ...
      children: [
        {
          style: {
            ...
            backgroundImage: "url('" + FrontAboutJs.binaryPath + "/" + contents0[i].image + "')",
  ...
  createNode({
    mother: whiteBlock2,
    mode: "img",
    attribute: { src: FrontAboutJs.binaryPath + "/" + contents2.image },
    style: {
      display: "block",
      position: "relative",
      width: String(100) + '%',
  ...
  for (let i = 0; i < contents3.children.length; i++) {
    ...
    createNode({
      ...
      style: {
        ...
        marginBottom: desktop ? String(contents3BoxBetween) + ea : String(contents3BoxBetween / 2) + ea,
        backgroundImage: "url('" + FrontAboutJs.binaryPath + "/" + contents3.children[i].image + "')",
        backgroundSize: "100% auto",
        backgroundPosition: "50% 50%",
      }
    })
  ...
  createNode({
    mother: rightBox4,
    mode: "img",
    attribute: { src: FrontAboutJs.binaryPath + "/" + contents4.image },
    style: {
      display: "block",
      width: withOut(0 * 2, ea),
    }
...
FrontAboutJs.prototype.launching = async function (loading) {
  ...
  try {
    ...
    console.log(err);
    await GeneralJs.ajaxJson({ message: "FrontAboutJs.launching 에러 일어남 => " + err.message }, "/errorLog");
  }
}
