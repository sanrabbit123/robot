...
DesignerExplanationJs.binaryPath = FRONTHOST + "/middle/aspirant";
...
DesignerExplanationJs.prototype.insertInitBox = async function () {
  ...
  try {
    ...
    subTitleSize = <%% 21, 18, 17, 15, 3.6 %%>;
    subTitleWeight = <%% 800, 800, 800, 800, 800 %%>;
    subTitleMarginTop = <%% 8, 8, 7, 6, 2.2 %%>;
...
DesignerExplanationJs.prototype.insertFifthBox = async function () {
  const instance = this;
  const { withOut, returnGet, createNode, colorChip, colorExtended, isMac, isIphone, svgMaker, serviceParsing, dateToString, stringToDate, findByAttribute, autoHypenPhone, setQueue, uniqueValue, homeliaisonAnalytics } = GeneralJs;
  const { ea, media, baseTong, standardWidth, naviHeight } = this;
  const mobile = media[4];
...
DesignerExplanationJs.prototype.insertSixthBox = async function () {
  const instance = this;
  const { withOut, returnGet, createNode, colorChip, colorExtended, isMac, isIphone, svgMaker, serviceParsing, dateToString, stringToDate, findByAttribute, autoHypenPhone, setQueue, uniqueValue, homeliaisonAnalytics } = GeneralJs;
  const { ea, media, baseTong, standardWidth, naviHeight } = this;
  const mobile = media[4];
    ...
    reviewContents = [
      {
        ...
        image: DesignerExplanationJs.binaryPath + "/peopleTarget0.png",
      },
      {
        review: (mobile ? [
        ...
        image: DesignerExplanationJs.binaryPath + "/peopleTarget1.png",
      },
      {
        review: (mobile ? [
          "홈리에종만의 특별한 점이 있어요.",
        ...
        image: DesignerExplanationJs.binaryPath + "/peopleTarget2.png",
      },
    ]
...
DesignerExplanationJs.prototype.postIframeEvent = function () {
  const instance = this;
