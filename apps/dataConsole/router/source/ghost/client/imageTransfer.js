...
ImageTransferJs.binaryPath = FRONTHOST + "/middle/aspsetting";
...
ImageTransferJs.prototype.insertAspirantBox = function () {
  const instance = this;
  const { withOut, returnGet, createNode, colorChip, isMac, isIphone, setDebounce, sleep, svgMaker, serviceParsing, dateToString, stringToDate, findByAttribute, autoHypenPhone, setQueue, uniqueValue, homeliaisonAnalytics, ajaxJson, equalJson, cleanChildren } = GeneralJs;
    ...
    try {
      ...
      GeneralJs.stacks["addressEvent"] = async function (e) {
        try {
          ...
          await GeneralJs.ajaxJson({ message: "ImageTransferJs.addressEvent : " + e.message }, BACKHOST + "/errorLog");
        }
      }
  ...
  if (generalMode) {
    ...
    for (let i = 0; i < pictureNumber; i++) {
      createNode({
        mother: exampleTong,
        mode: "img",
        attribute: { src: ImageTransferJs.binaryPath + "/set/t" + String(i + 1) + ".jpg" },
        style: {
          display: "inline-block",
          position: "relative",
  ...
  createNode({
    ...
    mode: "img",
    attribute: { src: ImageTransferJs.binaryPath + "/set/t" + String(garoRepresentativePhotoNumber) + ".jpg" },
    style: {
      display: "inline-block",
      position: "relative",
  ...
  createNode({
    ...
    mode: "img",
    attribute: { src: ImageTransferJs.binaryPath + "/set/t" + String(seroRepresentativePhotoNumber0) + ".jpg" },
    style: {
      display: "inline-block",
      position: "relative",
  ...
  createNode({
    mother: seroExampleTong,
    mode: "img",
    attribute: { src: ImageTransferJs.binaryPath + "/set/t" + String(seroRepresentativePhotoNumber1) + ".jpg" },
    style: {
      display: "inline-block",
      position: "relative",
      height: String(seroImageHeight) + ea,
