...
DesignerAboutJs.prototype.insertRepresentativeFrontPhotosBox = async function () {
  const instance = this;
  const mother = this.mother;
  const { client, ea, baseTong, media, project, targetHref, totalContents, entireMode, normalMode } = this;
  const mobile = media[4];
    ...
    const mainContents = [
      {
        title: "대표 가로 사진 세팅",
        contents: [
          "디자이너님의 스타일을 가장 잘 보여주고 어필할 수 있는 가로형 사진 1장이 필요합니다! 가장 자신 있는 사진으로 1장을 구성해주세요!",
        ],
      },
    ];
    let paddingTop;
...
DesignerAboutJs.prototype.insertRepresentativeWordsBox = async function () {
  const instance = this;
  const mother = this.mother;
  const { client, ea, baseTong, media, project, targetHref, totalContents, entireMode, normalMode } = this;
  const mobile = media[4];
    ...
    const mainContents = [
      {
        title: "대표 키워드 세팅",
        contents: [
          "디자이너님의 장점을 가장 잘 설명하고 어필할 수 있는 키워드가 필요합니다! 가장 적합한 키워드를 선택해주세요!",
        ],
      },
    ];
    let paddingTop;
      ...
      await this.mother.ghostDesignerLaunching({
        ...
        local: async () => {
          try {
            ...
            await instance.insertRepresentativeFrontPhotosBox();
          } catch (e) {
