...
DataPatch.prototype.clientMap = function () {
  ...
  const map = {
    ...
    outspot: { name: "유출 시점", position: "requests.0.analytics.response.action", type: "string", items: [ "1차 응대 예정", "1차 응대 후 대기", "제안 발송 예정", "제안 피드백 대기", "제안 피드백 완료", "제안 후 대기", "연결 안 됨", "계약금 입금", "계약서 서명", "잔금 입금", "응대 종료", "해당 없음" ], searchBoo: true, },
    ...
    callHistory: { name: "연락 기록", position: "requests.0.analytics.date.call.history", type: "object", inputFunction: callHistoryInputFunction.toString().replace(/\}$/, '').replace(/function \(mother, input, callback\) \{/gi, ''), objectFunction: callHistoryToObject.toString().replace(/\}$/, '').replace(/function \(value, pastValue, vaildMode\) \{/gi, ''), searchBoo: false, },
...
DataPatch.prototype.projectMap = function () {
  ...
  const map = {
    ...
    callHistory: { name: "연락 기록", position: "process.call.history", type: "object", inputFunction: callHistoryInputFunction.toString().replace(/\}$/, '').replace(/function \(mother, input, callback\) \{/gi, ''), objectFunction: callHistoryToObject.toString().replace(/\}$/, '').replace(/function \(value, pastValue, vaildMode\) \{/gi, ''), searchBoo: false, },
