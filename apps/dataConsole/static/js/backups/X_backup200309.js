...
X.prototype.columns_moving = function () {
  ...
  function dragend_event(e) { e.target.style.opacity = ""; }
  function dragenter_event(e) { e.target.style.opacity = "0.4"; }
  function dragleave_event(e) { e.target.style.opacity = ""; }
  ...
  function drop_event(e) {
    ...
    X.ajax('/session_update', sessionupdate, function (a) { console.log(a); });
  }
  ...
  if (mege) {
    ...
    function drop_event(e) {
      ...
      if ( e.target.className === "rowdiv_back" ) {
        ...
        let sessionupdate = "title=" + instance.data.dbtitle + "&method=color&thisid=" + target.textContent + "&value=" + data;
        X.ajax('/session_update2', sessionupdate, function (a) { console.log(a); });
      }
    }
    for (let node of ddnodes) {
      node.addEventListener("dragstart", dragstart_event);
    ...
    X.ajax('/session_on', sessionon, function (data) {
        ...
        for (let stan of rows_row) {
          ...
          if (document.querySelector('.id_' + stan)) {
            ...
            this_target.style.background = rows[stan];
            this_target_init.style.background = rows[stan];
          }
        }
    });
  } else {
...
X.prototype.rowcards = function (title, mege1, mege2, mege3, excep = [753346, 753346, 753346]) {
  let instance = this;
  let box = {
    longtext: [ 'position:fixed;top:20%;left:3.5%;width:calc(93.4% - 86px);height:63.5%;', 16, 40 ],
    longtextplus: [ 'position:fixed;top:20%;left:3.5%;width:calc(93.4% - 6px);height:auto;', 16, 40 ],
  ...
  function eashorttext_event(e) {
    ...
    if (e.target.nodeName !== "ARTICLE" && e.target.nodeName !== "SECTION" && e.target.nodeName !== "TEXTAREA" && e.target.id !== "cancel_back" && this.c === 0) {
      ...
      (function (m) {
        ...
        //effect
        function effect_event(e) {
          subupdate_node.textContent = this.textContent;
    ...
    if (e.target.nodeName !== "ARTICLE" && e.target.nodeName !== "SECTION" && e.target.nodeName !== "IMG" && e.target.id !== "cancel_back" && this.c === 0) {
      ...
      (function (m) {
        ...
        function cancel_event(e) {
          ...
          while (m.firstChild) { m.removeChild(m.lastChild); }
          m.textContent = instance.data_flow.past_data[0];
          instance.rowcards('objectmaker', mege, false, m);
          m.style.cssText = "overflow: hidden;";
          ...
          if (document.getElementById('cancel_back')) {
            document.getElementById('cancel_back').remove();
            document.getElementById('objectbox').remove();
          }
          if (e.cancelable) { e.preventDefault(); }
        }
        //node add event
...
X.prototype.ajaxfunc = function () {
  let dataq = 'id=' + this.data.dbtitle + '&qquery=' + 'SELECT * FROM ' + this.data.dbtitle + ' ORDER BY ' + this.data.standard + ' DESC LIMIT 100;';
  this.ajaxajax(dataq);
}
...
X.prototype.key_search = function () {
  ...
  function search_input_event(e) {
    if (keycode_arr.indexOf(e.keyCode) !== -1) {
      ...
      for (let i of instance.data.colcol_arr) { where_query += i + " regexp '" + this.value + "' OR "; }
      where_query = where_query.slice(0, -4);
      let qqq1 = "";
      if (this.value !== "") { qqq1 = 'SELECT * FROM ' + instance.data.dbtitle + where_query + ' ORDER BY ' + instance.data.standard + ' DESC LIMIT 100;'; }
      else { qqq1 = 'SELECT * FROM ' + instance.data.dbtitle + ' ORDER BY ' + instance.data.standard + ' DESC LIMIT 100;'; }
