        ...
        if (instance.data.menu.hasOwnProperty(node_id) || instance.data.calendarplus.hasOwnProperty(node_id)) {
          ...
          function filter_event(e) {
            if (instance.data.sortquery.hasOwnProperty(m.id.replace(/_col$/g, ''))) {
              ...
              let where_query = " WHERE " + m.id.replace(/_col$/g, '') + " regexp '" + valval + "'";
              let qqq1 = 'SELECT * FROM ' + instance.data.dbtitle + where_query + ' ORDER BY ' + instance.data.standard + ' DESC;';
...
X.prototype.colorpalettes = function (mege) {
  ...
  if (mege === 'init') {
    ...
    function drop_event(e) {
      if (e.cancelable) { e.preventDefault(); }
      let target_id, target, sessionupdate;
      ...
      if ( e.target.className === "rowdiv_init_back" ) {
        ...
        X.ajax('/session_update3', sessionupdate, function (a) { console.log(a); });
...
X.prototype.ajaxfunc = function () {
  let dataq = 'id=' + this.data.dbtitle + '&qquery=' + 'SELECT * FROM ' + this.data.dbtitle + ' ORDER BY ' + this.data.standard + ' DESC LIMIT 120;';
  this.ajaxajax(dataq);
}
...
X.prototype.key_search = function () {
  ...
  function search_input_event(e) {
    if (keycode_arr.indexOf(e.keyCode) !== -1) {
      ...
      let qqq1 = "";
      if (this.value !== "") { qqq1 = 'SELECT * FROM ' + instance.data.dbtitle + where_query + ' ORDER BY ' + instance.data.standard + ' DESC;'; }
      else { qqq1 = 'SELECT * FROM ' + instance.data.dbtitle + ' ORDER BY ' + instance.data.standard + ' DESC LIMIT 120;'; }
      let dataq = 'id=' + instance.data.dbtitle + '&qquery=' + qqq1;
      instance.ajaxajax(dataq);
    }
  }
