  ...
  try {
    ...
    for (let j = 0; j < this.text.proposal.length; j++) {
      for (let i = 0; i < this.text.proposal[j].picture_settings.length - 1; i++) {
        tempObject = await this.mother.binaryRequest(ADDRESS.consoleinfo.host, this.text.proposal[j].picture_settings[i].imgSrc);
        await this.mother.fileSystem(`writeBinary`, [ `${this.options.home_dir}/result/photo${String(j) + String(i)}.jpg`, tempObject ]);
      }
    }
