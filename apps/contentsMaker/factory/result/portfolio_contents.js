...
ExecMain.prototype.create = function (order, boo) {
  var fileObj = new File(this.folder_list.svg.string + '/' + this.filename[boo] + ".ai");
  ...
  var contents_obj = {};
  for (var sub in this.text.contents[order]) { if (sub !== "title" && sub !== "photo_key") {
  ...
  if (this.text.contents[order].smalltalk_yn === "+ HomeLiaison's small talk" && boo === "desktop") {
    ...
    contents_obj.smalltalk_yn.nameItem.top = (-1 * contents_obj.main_contents.return_bottom()) + 1 - margin.desktop_hs_up;
    contents_obj.smalltalk_contents.nameItem.top = (-1 * contents_obj.smalltalk_yn.return_bottom()) + 1 - margin.desktop_hs_down;
  ...
  this_ai.save();
  for (var i = 0; i < this_ai.layer_select("contents").pageItems.length; i++) { this_ai.layer_select("contents").pageItems[i].createOutline(); }
...
ExecMain.prototype.start = function () {
  for (var b = 0; b < this.text.contents.length; b++) {
    this.create_init(b);
