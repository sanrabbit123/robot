...
Titles.prototype.createElements = function (doc, obj) {
  var rectRef = doc.pathItems.rectangle(this.mother.convertMillimeters(obj.height), 0, this.mother.convertMillimeters(obj.width), this.mother.convertMillimeters(obj.height));
  var areaTextRef = doc.textFrames.areaText(rectRef);
  areaTextRef.contents = this.text.sub_titles.main_title;
  for (var i = 0; i < areaTextRef.paragraphs.length; i++) {
...
Titles.prototype.newdoc_setting = function (obj) {
  var newdoc_setting = new DocumentPreset();
...
Titles.prototype.resultXLine = function (targetnode, boo) {
  var title_copy = targetnode.duplicate();
  var title_group = title_copy.createOutline();
  var resultX;
  var compoundsDetailIn = [];
  for (var i = 0; i < title_group.compoundPathItems.length; i++) { if (i !== title_group.compoundPathItems.length - 1) {
  ...
  if (boo === "left") {
    for (var i = 0; i < title_group.compoundPathItems.length; i++) { if (i !== title_group.compoundPathItems.length - 1) { if (Math.floor(title_group.compoundPathItems[i+1].visibleBounds[1]) - Math.floor(title_group.compoundPathItems[i].visibleBounds[1]) === compoundsDetailIn[0]) {
...
ExecMain.prototype.titles = new Titles(this.text);

ExecMain.prototype.createDoc = function () {
  app.documents.addDocument("", this.titles.newdoc_setting(this.titles.createSetting["main_title"]));
  var this_ai = app.activeDocument;
  this.titles.createElements(this_ai, this.titles.createSetting["main_title"]);
  this.titles.createLines(this_ai, this.titles.createSetting["main_title"]);
  return this_ai;
}

    
ExecMain.prototype.copyDoc = function (target) {
  var templateObj = new File(this.dir + "factory/template/portfolio/subtitles/" + target + ".ai");
  var template_ai = templateObj.get_file();
  var new_file_string = ((target === "word1003") ? this.folder_list.svg.string + "/word1003" + this.text.p_id + "_00" : this.folder_list.main.string + '/' + target + this.text.p_id);
  template_ai.saveAs(new File(new_file_string + ".ai"));
  template_ai.close();
  var fileObj = new File(new_file_string + ".ai");
  ...
  if (target === "word1003") {
    ...
    for (var z = 0; z < general_arr.length; z++) { general_arr[z].characterAttributes.fillColor = this.mother.colorpick(this.text.sub_titles["main_color_object"]["title"]); }
...
ExecMain.prototype.start = function () {
  ...
  this_ai.close();
  var sub_list = [ "word1003","moportivec","moportivecgaro","porhovec","porhovecgaro","portivec","portivecgaro","name","name2" ];
