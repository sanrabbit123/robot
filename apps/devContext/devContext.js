...
DevContext.prototype.launching = async function () {
  ...
  try {
    ...
    // await this.passiveAddressSync("c2109_aa89s");
    ...
    // const clients = await back.getClientsByQuery({}, { selfMongo, withTools });
    // const requests = clients.getRequestsTong();
    // let whereQuery, updateQuery;
    //
    // for (let { request, analytics, cliid, index } of requests) {
    //
    //   if (analytics.response.action.value === "선호 사진 대기") {
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.action"] = "스타일 체크 대기";
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    //     console.log(whereQuery);
    //   }
    //
    //   if (analytics.response.action.value === "계약금 대기") {
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.action"] = "계약금 안내";
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    //     console.log(whereQuery);
    //   }
    //
    //   if (analytics.response.action.value === "연결 안 됨") {
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.action"] = "해당 없음";
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    //     console.log(whereQuery);
    //   }
    //
    //   if (analytics.response.action.value === "응대 종료") {
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.action"] = "해당 없음";
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    ...
    //
    //
    //   if (analytics.response.outreason.values.includes("의견 조정 안 됨")) {
    //     analytics.response.outreason.values = analytics.response.outreason.values.filter((s) => { return s !== "의견 조정 안 됨" });
    //     analytics.response.outreason.values.push("가족 의견 불일치");
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.outreason"] = equalJson(JSON.stringify(analytics.response.outreason.values));
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    //     console.log(whereQuery);
    //   }
    //
    //
    //   if (analytics.response.outreason.values.includes("기간 부적합")) {
    //     analytics.response.outreason.values = analytics.response.outreason.values.filter((s) => { return s !== "기간 부적합" });
    //     analytics.response.outreason.values.push("기간 임박");
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.outreason"] = equalJson(JSON.stringify(analytics.response.outreason.values));
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    ...
    //   }
    //
    //   if (analytics.response.outreason.values.includes("기타 문제")) {
    //     analytics.response.outreason.values = analytics.response.outreason.values.filter((s) => { return s !== "기타 문제" });
    //     analytics.response.outreason.values.push("고객 미션 미응답");
    //     whereQuery = { cliid };
    //     updateQuery = {};
    //     updateQuery["requests." + String(index) + ".analytics.response.outreason"] = equalJson(JSON.stringify(analytics.response.outreason.values));
    //     await back.updateClient([ whereQuery, updateQuery ], { selfMongo });
    ...
    */
    ...
    // await this.getRawPortfolio("p144");
