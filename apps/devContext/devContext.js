...
DevContext.prototype.launching = async function () {
  ...
  try {
    ...
    */
    ...
    // const bodyText = (thisName) => { return [
    //   `안녕하세요, ${thisName} 개발자님!\n이번에 저희 홈리에종의 프론트 개발 직무에 지원해 주셔서 진심으로 감사드립니다. 면접을 통해 귀하의 열정과 끼, 능력을 볼 수 있어 매우 영광이었으며, 이번 기회를 통해 귀하를 만나게 된 것에 대해 매우 기쁘게 생각합니다.`,
    //   `우리는 모든 지원자들의 자료를 심사하고 면접을 통해 깊게 고민한 끝에, 아쉽게도 이번에는 귀하를 선발하지 못하기로 결정하였습니다. 이 결정은 귀하의 능력이나 가치를 반영하는 것이 아니라, 단지 이번 회사의 특정 요구사항과 일치하지 않았음을 의미하는 것입니다.`,
    //   `저희는 귀하의 훌륭한 경력과 능력을 확신하며, 귀하의 성공이 못할 곳 없음을 알고 있습니다. 이 결과가 귀하의 경력을 위한 더 나은 기회로 이어지길 바랍니다.\n다시 한 번 귀하의 지원에 감사드리며, 향후 시 지원하실 기회가 있다면 그 때에는 좀 더 합당한 결과가 있기를 바랍니다.`,
    // ].join("\n\n") }
    // await human.sendSms({
    //   to: "01040092710",
    //   body: bodyText("김지혜"),
