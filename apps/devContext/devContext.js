...
DevContext.prototype.cxDashboardSync = async function (clientUpdate = true) {
  const instance = this;
  const { equalJson, dateToString, stringToDate } = this.mother;
  ...
  try {
    ...
    await sheets.update_value_inPython(sheetsId, sheetsName, contract.map((str) => { return [ str ] }), [ columns.findIndex((str) => { return /계약/gi.test(str) }), 1 ]);
    ...
    for (let i = 0; i < totalRows.length; i++) {
      if (totalRows[i][1] === "계") {
        ...
        if (/^O/gi.test(totalRows[i][columns.findIndex((str) => { return /계약/gi.test(str) })])) {
...
DevContext.prototype.launching = async function () {
  ...
  try {
    ...
    // await this.cxDashboardSync(false);



    await findCode("w3c-hr-time")
