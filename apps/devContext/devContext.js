  ...
  try {
    ...
    */
    ...
    const managerContract = async () => {
      ...
      projectsRaw = await back.getProjectsByQuery({ desid: { $regex: "^d" } }, { selfMongo });
      projects = projectsRaw.toNormal().filter((obj) => {  return obj.process.contract.first.date.valueOf() >= (new Date(2000, 0, 1)).valueOf() });
      ...
      for (let name of keys) {
        ...
        arr.push(result[name].length);
        arr.push(total[name].length === 0 ? 0 : (Math.round((result[name].length / total[name].length) * 10000) / 100));
        arr.push(totalLength === 0 ? 0 : (Math.round((total[name].length / totalLength) * 10000) / 100));
        arr.push(resultLength === 0 ? 0 : (Math.round((result[name].length / resultLength) * 10000) / 100));
        matrix.push(arr);
      }
      ...
      for (let [ year, month ] of dateArr) {
        ...
        tempClient = equalJson(JSON.stringify(clients));
        tempProject = projectsRaw.toNormal();

        tempClient = tempClient.filter((obj) => { return obj.timeline.getFullYear() === year && obj.timeline.getMonth() + 1 === month });
        tempCliidArr = tempClient.map((obj) => { return obj.cliid });

        matrix.push([
          "전체",
          tempClient.length,
          tempProject.filter((obj) => { return tempCliidArr.includes(obj.cliid); }).length,
          tempProject.filter((obj) => { return obj.process.contract.first.date.getFullYear() === year && obj.process.contract.first.date.getMonth() + 1 === month }).length
        ]);
