...
ResourceMaker.prototype.portfolio_maker = async function () {
  ...
  try {
    ...
    let result_string = `module.exports = ${JSON.stringify(result_obj, null, 3)}`;
    console.log(await this.mother.fileSystem("write", [ `${process.cwd()}/apps/contentsMaker/resource/${result.get("p_id")}.js`, result_string ]));
...
ResourceMaker.prototype.launching = async function () {
  try {
    ...
    let note = new AppleAPIs({ folder: "portfolio", subject: this.p_id });
