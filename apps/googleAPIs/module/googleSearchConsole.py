...
class GoogleSearchConsole:
    ...
    def basicImpressions(self, siteUrl, startDate, endDate):
        request = { "startDate": startDate, "endDate": endDate, "dimensions": [ "date" ] }
        response = self.app.searchanalytics().query(siteUrl="https://" + siteUrl, body=request).execute()
        return response

    def queryImpressions(self, siteUrl, startDate, endDate):
