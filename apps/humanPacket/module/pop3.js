...
const Pop3Client = function (port, host, options = {}) {
  ...
  let callback = (resp, data) => {
    ...
    if (resp === false) {
      ...
      let banner = data.trim();
      let bannerComponents = banner.split(" ");
  ...
  this.starttls = function(options) {
    ...
    let sslcontext = require('crypto').createCredentials(options);
    let pair = tls.createSecurePair(sslcontext, false);
    let cleartext = pipe(pair);

    pair.on('secure', function() {

      let verifyError = pair.ssl.verifyError();
  ...
  function onData(data) {
    ...
    if (checkResp === false) {
      ...
      if (multiline === true && (response === false || bufferedData.substr(bufferedData.length-5) === "\r\n.\r\n")) {
        ...
        let bufferedDataCopy = bufferedData;
    ...
    if (type === "PLAIN") {
      ...
      self.setCallback(function(resp, data) {
        ...
        if (resp === false) self.emit("auth", resp, "Server responded -ERR to AUTH CRAM-MD5", data);
          ...
          let challenge = new Buffer(data.trim().substr(2), "base64").toString();
          let hmac = crypto.createHmac("md5", password);
          let response = new Buffer(username + " " + hmac.update(challenge).digest("hex")).toString("base64");
