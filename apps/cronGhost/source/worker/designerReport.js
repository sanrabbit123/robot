 ...
 */
const dayId = [
  "d091", // 'd091'은 매일 오전 9시 10분에 실행되어야 함을 나타냅니다.
];
...
const worker = async function (package) {
  ...
  try {
     ...
     * 이 요청은 디자이너 제안서의 실시간 업데이트를 처리하기 위한 작업을 수행합니다.
     * - URL: address 객체의 officeinfo.ghost.host에 정의된 호스트를 사용합니다.
     * - 엔드포인트: /updateDesignerProposalRealtime
     * - 요청 본문: { report: 1 } - 제안서 업데이트에 필요한 데이터를 포함합니다.
    ...
    /**
     * 요청이 성공적으로 완료되면 errorLog 메서드를 사용하여 작업이 완료되었음을 기록합니다.
     * - 메시지: "designer reporting done" - 작업이 완료되었음을 나타내는 메시지입니다.
     */
     ...
     * - 오류 메시지: "designer reporting error : " + e.message - 발생한 오류의 메시지를 포함합니다.
