...
const worker = async function (package) {
  ...
  try {
    /**
     * requestSystem 메서드를 사용하여 첫 번째 URL에 HTTP POST 요청을 보냅니다.
     * 이 요청은 'frontReflection' 작업을 처리하기 위한 것입니다.
     * - URL: address 객체의 officeinfo.ghost.host에 정의된 호스트와 포트 3000을 사용합니다.
     * - 엔드포인트: /frontReflection
    ...
    /**
     * requestSystem 메서드를 사용하여 두 번째 URL에 HTTP POST 요청을 보냅니다.
     * 이 요청은 'syncContentsTag' 작업을 처리하기 위한 것입니다.
     * - URL: address 객체의 officeinfo.ghost.host에 정의된 호스트와 포트 3000을 사용합니다.
     * - 엔드포인트: /syncContentsTag
     * - 요청 본문: { data: null } - 데이터 없이 빈 요청을 보냅니다.
     * - 요청 헤더: Content-Type을 application/json으로 설정하여 JSON 형식의 데이터를 주고받음을 명시합니다.
     * @returns {Promise<void>}
     */
    ...
    /**
     * 모든 작업이 성공적으로 완료된 경우 true를 반환합니다.
     * @returns {Promise<boolean>} 작업이 성공적으로 완료되었음을 나타냅니다.
     */
    return true;
  } catch (e) {
    /**
     ...
     * - 오류 메시지: "front reflection error : " + e.message - 발생한 오류의 메시지를 포함합니다.
