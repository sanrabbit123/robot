 ...
 * 이 작업은 디자인 제안서를 동기화하는 HTTP 요청을 보냅니다.
 */
 ...
 */
const dayId = [
  "d061", // 'd061'은 매일 오전 6시 10분에 실행되어야 함을 나타냅니다.
];
...
const worker = async function (package) {
  ...
  try {
     ...
     * 이 요청은 /syncDesignProposal 작업을 수행하여 디자인 제안서 정보를 동기화합니다.
     * - URL: address 객체의 officeinfo.ghost.host에 정의된 호스트를 사용합니다.
     * - 포트: 3000
     * - 엔드포인트: /syncDesignProposal
    ...
    /**
     * 동기화 작업이 완료되면, 로그를 남깁니다.
     * errorLog 메서드를 사용하여 "sync design proposal request done" 메시지를 기록합니다.
     ...
     * - 오류 메시지: "sync design proposal request error : " + e.message - 발생한 오류의 메시지를 포함합니다.
