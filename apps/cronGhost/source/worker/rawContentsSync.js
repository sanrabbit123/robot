 ...
 */
const dayId = [
  "d193" // 'd193'는 매일 오후 7시 30분에 실행되어야 함을 나타냅니다.
];
...
const worker = async function (package) {
  ...
  try {
     ...
     * 이 요청은 원시 콘텐츠 동기화를 처리하기 위한 것입니다.
     * - URL: address 객체의 secondinfo.host에 정의된 호스트를 사용합니다.
     * - 포트: 3003
     * - 엔드포인트: /rawContentsSync
     * - 요청 본문: { data: null } - 요청에 포함될 데이터(현재는 null)를 전송합니다.
     * - 요청 헤더: Content-Type을 application/json으로 설정하여 JSON 형식의 데이터를 주고받음을 명시합니다.
     * @returns {Promise<void>}
     */
    await requestSystem("https://" + address.secondinfo.host + ":3003/rawContentsSync", { data: null }, { headers: { "Content-Type": "application/json" } });
     ...
     * - 오류 메시지: "raw contents sync error : " + e.message - 발생한 오류의 메시지를 포함합니다.
     * @returns {Promise<boolean>} 요청이 실패하면 false 값을 반환합니다.
     */
    await emergencyAlarm("raw contents sync error : " + e.message);
    return false;
  }
}
