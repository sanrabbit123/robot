 ...
 */
const dayId = [
  "d131", // 'd131'는 매일 오후 1시 10분에 실행되어야 함을 나타냅니다.
];
...
const worker = async function (package) {
  ...
  try {
     ...
     * - 엔드포인트: /noticeAspirantContractYesterday
     * - 요청 본문: { data: null }
     * - 요청 헤더: Content-Type을 application/json으로 설정합니다.
     * @returns {Promise<boolean>} 요청이 성공하면 true를 반환합니다.
     */
    await requestSystem("https://" + address.secondinfo.host + ":3003/noticeAspirantContractYesterday", { data: null }, { headers: { "Content-Type": "application/json" } });
    return true;
  } catch (e) {
    /**
     * 요청이 실패하면 catch 블록에서 예외를 처리합니다.
     * emergencyAlarm 메서드를 사용하여 비상 알람을 트리거하고, 오류 메시지를 로깅합니다.
     * - 오류 메시지: "aspirant contract error : " + e.message
