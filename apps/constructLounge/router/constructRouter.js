const ConstructRouter = function (MONGOC, MONGOLOCALC, MONGOCONSOLEC, kakaoInstance) {
  ...
  this.host = this.address.constructinfo.host;
  this.mongo = MONGOC;
  ...
  this.vaildHost = [
    ...
    this.address.constructinfo.host,
...
ConstructRouter.prototype.fireWall = function (req) {
  const instance = this;
...
ConstructRouter.prototype.rou_get_First = function () {
  const instance = this;
...
ConstructRouter.prototype.rou_post_storeHoliday = function () {
  const instance = this;
  ...
  obj.func = async function (req, res, logger) {
    ...
    try {
      ...
      logger.error("Construct lounge 서버 문제 생김 (rou_post_storeHoliday): " + e.message).catch((e) => { console.log(e); });
...
ConstructRouter.prototype.setMembers = async function () {
  const instance = this;
  const back = this.back;
  ...
  try {
    ...
    await logger.error("Construct lounge 서버 문제 생김 (setMembers): " + e.message);
    console.log(e);
  }
}

ConstructRouter.prototype.getAll = function () {
  let result, result_arr;

  result = { get: [], post: [] };
  result_arr = Object.keys(ConstructRouter.prototype);
