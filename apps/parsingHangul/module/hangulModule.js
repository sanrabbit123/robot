...
let CONSONANTS_HASH, CHO_HASH, JUNG_HASH, JONG_HASH, COMPLEX_CONSONANTS_HASH, COMPLEX_VOWELS_HASH;

const _makeHash = function (array) {
...
const _makeComplexHash = function (array) {
    let length = array.length,
        hash = {},
        code1,
        code2
...
const disassembleToString = function (str) {
...
const assemble = function (array) {
    ...
    _makeHangul(length - 1);

    return result.join('');
};

const search = function (a, b) {
    /* a 와 b 를 disassemble한 후 문자열로 반환해 ad, bd에 각각 저장 */
    let ad = disassemble(a).join(''),
...
const rangeSearch = function (haystack, needle) {
    let hex = disassemble(haystack).join(''),
    ...
    function findStart(index) {
        for (let i = 0, length = 0; i < grouped.length; ++i) {
...
const Searcher = function (string) {
    this.string = string;
...
const endsWithConsonant = function (string) {
    if (typeof string === 'object') {
        string = string.join('');
    }

    let code = string.charCodeAt(string.length - 1);
...
const endsWith = function (string, target) {
