  ...
  const getPrinterName = function () {
    ...
    return new Promise((resolve, reject) => {
      lpstat.stdout.on("data", (data) => {
        ...
        const printerRaw = arr.find((i) => { return /^printer/gi.test(i) && /EPSON_WF_2650_Series_homeliaisonPrinter/gi.test(i); });
...
Ghost.prototype.ghostRouter = function (needs) {
  ...
  funcObj.post_voice = {
    ...
    func: function (req, res) {
      ...
      requestSystem("http://" + instance.address.officeinfo.ghost.second.host + ":3000/textToVoice", { text }, { headers: { "Content-Type": "application/json" } }).catch((err) => {
        console.log(err);
      });
      res.send(JSON.stringify({ message: "will do" }));
    }
